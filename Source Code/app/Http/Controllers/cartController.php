...
class cartController extends Controller
{
    public function index()
    {
        ...
        if ($cart && count($cart)) {
            ...
            foreach ($cart as $product) {
                //$product['product'] == ['product_id']; by mistake
                $productDetails = Product::where("id", '=', $product['product'])->get();
                $cartPro = [];
                $cartPro["product"] = $productDetails[0];
                if ($cartPro["product"]->discount != 0) {
                    $cartPro["product"]->price = round(($cartPro["product"]->price) - (($cartPro["product"]->discount / 100) * ($cartPro["product"]->price)));
                }
    ...
    public function addToCart(Request $req)
    {
        ...
        // toast('Product add to the cart', 'success');
        alert()->success('Your product add to the cart');
    ...
    public function Checkout()
    {
        ...
        return view('public.checkOut', compact('cartDetails', 'total', 'customer'));
    }
}
